...
function Nav() {
  ...
  return (
    <nav className="bg-slate-900 text-white px-6 py-3 flex items-center justify-between sticky top-0 z-50 border-b border-slate-800">
      <a href="/" className="text-lg font-bold text-blue-400 whitespace-nowrap">NextEra Estate<sup className="text-xs text-blue-300 ml-0.5">™</sup></a>
      {user && (
        <div className="flex items-center gap-1 text-xs">
          <a href="/dashboard" className="px-3 py-2 rounded hover:bg-slate-800 whitespace-nowrap">Dashboard</a>
          <div className="relative">
            <button onClick={() => { setPlanOpen(o => !o); setToolsOpen(false); }} className="px-3 py-2 rounded hover:bg-slate-800 flex items-center gap-1">
          ...
          <div className="relative">
            <button onClick={() => { setToolsOpen(o => !o); setPlanOpen(false); }} className="px-3 py-2 rounded hover:bg-slate-800 flex items-center gap-1">
...
function Grief() {
  ...
  const send = async () => {
    ...
    } catch { setMessages(prev => [...prev, { role: 'ai', text: 'I apologize, temporarily unavailable.' }]); }
    setLoading(false);
  };
  return (
    <div className="min-h-screen bg-slate-50">
...
function Family() {
  ...
  const [form, setForm] = useState({ firstName: '', lastName: '', email: '', relationship: '', role: 'beneficiary' });
  const [showForm, setShowForm] = useState(false);
  useEffect(() => {
    if (!token) return;
    fetch(`${API}/api/family`, { headers: { Authorization: `Bearer ${token}` } }).then(r => r.json()).then(d => Array.isArray(d) ? setMembers(d) : null).catch(() => {});
  }, [token]);
  const add = async () => {
    if (!form.firstName || !form.email) return;
    try {
      const r = await fetch(`${API}/api/family`, { method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` }, body: JSON.stringify(form) });
      const d = await r.json();
      if (d.id) { setMembers(prev => [...prev, d]); setShowForm(false); setForm({ firstName: '', lastName: '', email: '', relationship: '', role: 'beneficiary' }); }
    } catch {}
  };
  return (
    <div className="min-h-screen bg-slate-50">
      ...
      <div className="max-w-4xl mx-auto px-6 py-10">
        <div className="flex items-center justify-between mb-8">
          <div><h1 className="text-3xl font-bold text-slate-800">Family Members</h1><p className="text-slate-500">Manage beneficiaries and heirs</p></div>
          <button onClick={() => setShowForm(!showForm)} className="bg-blue-600 text-white px-5 py-2 rounded-lg hover:bg-blue-700">+ Add Member</button>
        </div>
        {showForm && (
          <div className="bg-white rounded-xl border border-slate-200 p-6 mb-6 grid grid-cols-2 gap-4">
            {[{label:'First Name',key:'firstName',ph:'Jane'},{label:'Last Name',key:'lastName',ph:'Smith'},{label:'Email',key:'email',ph:'jane@email.com'},{label:'Relationship',key:'relationship',ph:'Spouse, Child...'}].map(f => (
              <div key={f.key}><label className="block text-xs font-medium text-slate-600 mb-1">{f.label}</label>
              <input value={(form as any)[f.key]} onChange={e => setForm(p => ({...p,[f.key]:e.target.value}))} placeholder={f.ph} className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-blue-400" /></div>
            ))}
            <div className="col-span-2 flex gap-3">
              <button onClick={add} className="bg-blue-600 text-white px-5 py-2 rounded-lg text-sm hover:bg-blue-700">Add</button>
              <button onClick={() => setShowForm(false)} className="border border-slate-200 px-5 py-2 rounded-lg text-sm">Cancel</button>
            </div>
          </div>
        )}
        {members.length === 0
          ? <div className="bg-white rounded-xl border border-slate-200 p-12 text-center text-slate-400">No family members yet.</div>
          : <div className="space-y-3">{members.map(m => (
              <div key={m.id} className="bg-white rounded-xl border border-slate-200 p-4 flex items-center gap-4">
                <div className="w-10 h-10 bg-blue-100 text-blue-600 rounded-full flex items-center justify-center font-semibold">{m.first_name?.[0] || m.firstName?.[0]}</div>
                <div><div className="font-medium text-slate-800">{m.first_name || m.firstName} {m.last_name || m.lastName}</div><div className="text-xs text-slate-400">{m.email} • {m.relationship}</div></div>
              </div>
            ))}</div>}
      </div>
    </div>
